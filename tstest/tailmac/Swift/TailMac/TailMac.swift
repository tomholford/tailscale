...
extension Tailmac {
    struct Run: ParsableCommand {
        ...
        mutating func run() {
            let process = Process()
            let stdOutPipe = Pipe()
            let appPath = "./TailMac.app/Contents/MacOS/TailMac"
            ...
            if !FileManager.default.fileExists(atPath: appPath) {
                fatalError("Could not find TailMac.app.  This must be co-located with the tailmac utility")
            }
...
extension Tailmac {
    struct Clone: ParsableCommand {
        @Option(help: "The vm identifer") var id: String
        @Option(help: "The vm identifer for the cloned vm") var targetId: String
...
extension Tailmac {
    struct Create: ParsableCommand {
        @Option(help: "The vm identifer.  Each VM instance needs a unique ID.") var id: String
        @Option(help: "The mac address of the socket network interface") var mac: String?
        @Option(help: "The port for the virtio socket device") var port: String?
        @Option(help: "The named socket for the socket network interface") var sock: String?
        @Option(help: "The desired RAM in bytes") var mem: String?
        @Option(help: "The ethernet address for a standard NAT adapter") var ethermac: String?
        @Option(help: "The image name to build from.  If ommitted we will use RestoreImage.ipsw in ~/VM.bundle and download it if needed") var image: String?
