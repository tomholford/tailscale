...
type groSupport bool

const (
	disableGRO groSupport = false
	enableGRO  groSupport = true
)

func newLinkEndpoint(size int, mtu uint32, linkAddr tcpip.LinkAddress, gro groSupport) *linkEndpoint {
	le := &linkEndpoint{
		gro: gro,
		q: &queue{
...
func (l *linkEndpoint) getGRO() *gro.GRO {
	l.mu.RLock()
	d := l.dispatcher
	l.mu.RUnlock()
	g := gro.NewGRO()
